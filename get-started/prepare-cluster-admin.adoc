...
NOTE: Ab ONTAP 9.19.1 können SVM-Administratoren ihre eigenen SVMs über ein dediziertes System Manager-Dashboard überwachen und verwalten. Erfahren Sie mehr über die https://docs.netapp.com/us-en/ontap/concept-svm-admin-dashboard-sm.html["ONTAP System Manager SVM-Administrator-Dashboard"^]
...
Bei der Konfiguration und Nutzung Ihres Clusters gelten in AFX verschiedene Beschränkungen, darunter Mindest- und Höchstwerte. Diese Beschränkungen lassen sich in mehrere Kategorien einteilen.


CAUTION: Bevor Sie einen Betriebsparameter Ihres AFX-Speichersystems erhöhen, sollten Sie die möglichen Auswirkungen auf Ihre Anwendungen und den gesamten Cluster prüfen. Beispielsweise erhöht die Erweiterung der Anzahl der Controller-Knoten zwar die Gesamtkapazität, kann aber auch den Datenverkehr im Clusternetzwerk steigern. Es ist wichtig, die Konsequenzen dieser Konfigurationsänderungen und deren Auswirkungen auf Ihre Umgebung zu verstehen.

Controllerknoten pro Cluster:: Jeder AFX-Cluster muss mindestens vier Knoten umfassen. Die maximale Anzahl an Knoten variiert je nach ONTAP Release; beispielsweise erlaubt die 9.19.1 RC Release bis zu 32 Knoten pro AFX-Cluster.
...
Cluster-Switches:: Sie benötigen mindestens zwei Switches in Ihrem Cluster-Speichernetzwerk.  Die maximal zulässige Anzahl wird anhand der Gesamtzahl der Controller-Knoten im Cluster bestimmt.


Sie sollten die Details im NetApp Hardware Universe und im Interoperability Matrix Tool überprüfen, um die Leistungsfähigkeit Ihres AFX-Clusters zu bestimmen. Überprüfen Sie außerdem link:../release-notes/changes-to-limits-defaults.html["Änderungen an ONTAP Grenzwerten und Standardeinstellungen"] im Abschnitt der AFX-Versionshinweise.
